...
# Pointer 
In rust pointer must always points to the some valid location there are no null references,Rust has optional pointer ,like option owned box, `Option<Box<T>>` `<Box<T>>is a type of pointer that uniquely owns a heap if allocation of type T.Boxes provide the ownership of the allocation .
...
# The union
# Prims Algorithn for tge minium spanning tress 
